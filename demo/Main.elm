...
import Html exposing ( Html, main_, text, img, a, span, i )
...
urlOf model =
  let
    (selectedTab, selectedNav) = model.selectedTab
    (tab, navList) =
      defTabs
  ...
  let
    ...
    tab =
      case List.head parts of
        Maybe.Just value -> getTabId value
        Maybe.Nothing -> 0
    nav =
      case List.length parts of
        2 -> getNavId tab (parts |> List.drop 1 |> List.head |> Maybe.withDefault "")
        _ -> 0
  in
    [SelectTab (tab,nav)]
  ...
  let
    tabElems =
      defTabs
        ...
        |> Array.indexedMap (\i name ->
                               tab (i == (model.selectedTab |> Tuple.first)) [] [href ("#" ++ name)]
                               [ text name ])
        |> Array.toList
  in
    container []
...
demoTopNavBar =
  navbar navbarModifiers []
    ...
    , navbarEnd [] 
        [ navbarItemLink False [ href "https://github.com/surprisetalk/elm-bulma" ]
            [ Elements.icon Standard [] [ i [class "fa fa-github"] [] ]
            , Html.span [] [ text "Source" ]
            ]
        , navbarItemLink False [ href "http://package.elm-lang.org/packages/surprisetalk/elm-bulma/latest" ]
